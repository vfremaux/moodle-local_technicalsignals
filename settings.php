...
if ($hassiteconfig) {
    ...
    $settings = new admin_settingpage('local_technicalsignals', $label, 'local/technicalsignals:manage');
    ...
    if (@$CFG->vmasterdbname == $CFG->dbname) {
        ...
        $desc = get_string('configadminmessage_desc', 'local_technicalsignals');
        $default = '';
        $settings->add(new admin_setting_configtext($key, $label, $desc, $default, PARAM_RAW, 80));
    ...
    $settings->add(new admin_setting_configtext($key, $label, $desc, $default, PARAM_RAW, 40));
}
